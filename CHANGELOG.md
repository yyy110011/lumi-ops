...
- **Atomic Root Agent Rule Write** — `syncRootAgentRule` now writes `.agents/rules/lumi-ops-root-agent.md` to a temp file and renames it into place. The previous truncate-then-write could expose an empty rule file to a concurrent reader or writer (the config-change listener and a direct sync can overlap), which also surfaced as a flaky Ubuntu integration test.

### 🔧 Improvements
- **Test Runner for VS Code ≥ 1.137** — Upgraded `@vscode/test-cli` to 0.0.15 and `@vscode/test-electron` to 3.x. VS Code 1.110 renamed the macOS binary from `Electron` to `Code` and 1.137 dropped the compatibility symlink, so the old runner failed with `spawn …/Contents/MacOS/Electron ENOENT` before any test ran. CI now runs on Node 22, which the new runner requires (Node 20 is end-of-life).
