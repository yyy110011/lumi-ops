...
describe('ShadowItem', () => {
  ...
  describe('currentBranch role', () => {
    it('shows ★ when current window is at root (no currentWorkspacePath)', () => {
      const item = new ShadowItem('main', NONE, makeClone({ branch: 'main', path: '/repo', isShadow: false }), 'currentBranch', '/ext', undefined);
      expect(item.description).toBe('Worktree Root · ★');
    });

    it('does NOT show ★ when current window is in a clone', () => {
      const item = new ShadowItem('main', NONE, makeClone({ branch: 'main', path: '/repo', isShadow: false }), 'currentBranch', '/ext', '/repo.worktrees/feat/test');
      expect(item.description).toBe('Worktree Root');
    });
    ...
    it('shows conflict prefix when hasConflict is true', () => {
      const item = new ShadowItem('main', NONE, makeClone({ branch: 'main', path: '/repo', isShadow: false, hasConflict: true }), 'currentBranch', '/ext', undefined);
  ...
  describe('shadowClone role', () => {
    ...
    it('always uses cycleReviewStatus command', () => {
      const item = new ShadowItem('feat/test', NONE, makeClone(), 'shadowClone', '/ext', undefined);
