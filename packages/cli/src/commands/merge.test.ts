...
describe('merge', () => {
  ...
  it('should use process.cwd() when root is not provided', async () => {
    // merge uses `options.root ? path.resolve(options.root) : process.cwd()`
    // When root is provided (our normal case), it resolves it;
    // this test just verifies merge doesn't crash with a valid root
